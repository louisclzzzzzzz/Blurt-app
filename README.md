...
- **Phase 7 (pilote dictée live nutrition)** : fait — capture vocale en flux continu pour le domaine nutrition uniquement (voir `DICTEE_LIVE_NUTRITION.md` pour le périmètre et l'architecture cible), en 4 sous-phases séquentielles, chacune vérifiée en conditions réelles avant la suivante.
  - **7A (WS + transcription brute)** : nouveau `WS /captures/stream` (`routers/capture_stream.py`) relayant un flux audio PCM (16-bit LE, 16kHz, mono) vers Voxtral Realtime (`services/transcription.py::stream_realtime_transcription`, SDK `mistralai[realtime]`) et retransmettant les évènements de transcription en direct. Nouveau statut `STREAMING` sur `VoiceCapture`. Frontend : hook `useLiveCapture` (AudioWorklet + resampling 16kHz — pas `MediaRecorder`, qui ne sort que de l'audio compressé) et écran `LiveCaptureScreen`, point d'entrée additif (lien sous `MicButton`) qui ne remplace pas le flux existant. Audio archivé sur Supabase Storage (encapsulé en WAV) à la fin de la dictée, capture finalisée en `PENDING_VALIDATION`.
    **Constat empirique important** : `transcription.segment` (segment finalisé côté Voxtral Realtime) ne se déclenche jamais en pratique pour une dictée mono-locuteur, même sur des dictées de plus de 10s avec de vraies pauses de silence — probablement un évènement pensé pour la diarisation multi-locuteurs (champ `speaker_id`), pas pour le découpage d'une dictée. Le découpage en segments naturels repose donc sur une heuristique côté serveur (ponctuation de fin de phrase OU ~800ms d'inactivité sur le flux de `transcription.text.delta`), pas sur cet évènement du SDK.
  - **7B (extraction incrémentale par opérations)** : nouveau schéma par diff (`schemas/extraction.py::DraftOperation`/`DraftOperations`, `services/extraction.py::extract_operations`, nouveau prompt dédié à la tâche de diff) appelé sur chaque segment naturel avec l'état courant du brouillon actif. Nouvelle table `capture_draft_items` (migration `0006`, statut `active`/`removed`). Vérifié en réel : une correction en langage libre ("en fait c'était 200 grammes") modifie bien l'item existant plutôt que d'en créer un nouveau ; un aliment ajouté puis retiré dans la même dictée (sans découpage net entre les deux) n'apparaît jamais, le modèle calculant correctement l'effet net même sur un segment composé.
  - **7C (matching + validation)** : chaque opération add/modify passe par `_resolve_food_item` (`routers/captures.py`, même fonction que le flux batch, pas de logique dupliquée). `POST /captures/{id}/validate` réutilisé sans aucune modification. Vérifié en réel : capture créée par le flux streaming validée avec succès (macros calculées correctement à partir du candidat matché).
  - **7D (UI définitive)** : `LiveCaptureScreen` réutilise `FoodItemRow` tel quel (cœur de résolution extrait dans `lib/buildEditableItems.ts::buildEditableFoodItem`, partagé avec le flux batch) pour l'édition/suppression manuelle en complément de la correction vocale ; fin de dictée réutilise `validateCapture` sans modification. Gestion basique des erreurs de connexion WS : une coupure inattendue en cours de dictée finalise la session avec ce qui a déjà été capturé plutôt que de bloquer l'écran (pas de reprise de session pour ce pilote).
    **Vérifié en conditions réelles avec un vrai navigateur** (pas seulement un client Python) : micro simulé par injection d'un `MediaStream` généré à partir d'un fichier audio réel (contournement du prompt de permission natif du navigateur, hors de portée de l'automatisation), pour exercer le pipeline `AudioWorklet` jamais testé jusque-là. Deux bugs réels trouvés et corrigés grâce à cet essai : (1) absence de filtre anti-repliement avant décimation dans le worklet — une interpolation linéaire seule à un ratio ~3:1 (48kHz→16kHz) laissait passer du repliement spectral, ayant concrètement fait entendre "pomme" comme "bombe" par Voxtral ; corrigé par une cascade de filtres passe-bas avant décimation. (2) si le pipeline de transcription meurt avant que le client ne signale la fin de la dictée (ex: connexion Mistral perdue), la boucle de réception WS continuait à accumuler de l'audio sans plus jamais le consommer ; corrigée en faisant courser la réception et la tâche de transcription (`asyncio.wait`), la session se finalisant immédiatement dans ce cas. Après ces correctifs : dictée reconnue correctement, matchée, éditable via `FoodItemRow`, validée avec succès (`POST /captures/{id}/validate` → 200), écran de confirmation affiché, aucune erreur console.
    **Limite non résolue** : la dégradation initialement observée tient en partie au double rééchantillonnage propre au harnais de test (décodage du fichier de test par le navigateur avant l'injection, en plus de la décimation du worklet) — un micro réel ne subit qu'une seule décimation. La qualité de reconnaissance avec un vrai micro reste à confirmer par l'utilisateur.
