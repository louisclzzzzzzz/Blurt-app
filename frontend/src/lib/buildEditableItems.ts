...
/** Cœur de la résolution par défaut d'un item aliment (choix d'une
 * correspondance connue / nouvel aliment), partagé entre le flux batch
 * (buildEditableItems) et la dictée live nutrition (LiveCaptureScreen, cf.
 * DICTEE_LIVE_NUTRITION.md Phase 7D) — même heuristique dans les deux cas. */
export function buildEditableFoodItem(item: FoodItemLike): EditableItem {
...
export function buildEditableItems(response: CaptureCreateResponse): EditableItem[] {
  return response.food_items.map(buildEditableFoodItem)
}
