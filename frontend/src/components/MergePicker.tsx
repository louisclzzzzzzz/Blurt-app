...
export function MergePicker({ domain, sourceId, sourceName, onMerged, onCancel }: MergePickerProps) {
  ...
  if (target !== null) {
    return (
      <div className="flex flex-col gap-4 w-full max-w-md px-4">
        <h2 className="text-lg font-medium">Confirmer la fusion</h2>
        <p className="text-sm">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={busy}
            className="flex-1 rounded-lg bg-red-600 text-white py-2 text-sm disabled:opacity-40"
          >
  ...
  return (
    <div className="flex flex-col gap-4 w-full max-w-md px-4">
      <h2 className="text-lg font-medium">Fusionner « {sourceName} »</h2>
      ...
      <button type="button" onClick={onCancel} className="text-xs text-neutral-400 underline w-fit">
        Annuler
      </button>
    </div>
  )
